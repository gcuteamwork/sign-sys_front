...
const tick=()=> {
    if( video.value.HAVE_ENOUGH_DATA===4){
        ...
        if (canvas2d.value === undefined) {
          canvas2d.value = dom.getContext("2d",{willReadFrequently: true}) as CanvasRenderingContext2D;
        }
...
.line {
  height: calc(100% - 2px);
  width: 100%;
  background: linear-gradient(180deg, rgba(110, 234, 251, 0.1) 43%, #2967cc 211%);
